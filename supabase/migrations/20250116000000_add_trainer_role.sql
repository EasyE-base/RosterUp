...
-- ----------------------------------------------------------------------------
-- 1. UPDATE PROFILES TABLE - Add 'trainer' to user_type enum
-- ----------------------------------------------------------------------------

-- First, create a new enum type with the trainer value
DO $$
BEGIN
  -- Check if the type already includes 'trainer'
  IF NOT EXISTS (
    SELECT 1 FROM pg_enum e
    JOIN pg_type t ON e.enumtypid = t.oid
    WHERE t.typname = 'user_type' AND e.enumlabel = 'trainer'
  ) THEN
    -- Add 'trainer' to the existing enum
...
-- ----------------------------------------------------------------------------
-- 6. STORAGE BUCKETS (Note: Must be created via Supabase Dashboard or API)
-- ----------------------------------------------------------------------------

-- Buckets to create in Supabase Dashboard:
-- 1. 'trainer-videos' - For intro videos and promo videos
--    - Public: true
--    - File size limit: 500MB
--    - Allowed MIME types: video/mp4, video/quicktime, video/webm
--
-- 2. 'trainer-photos' - For headshots and certification images
--    - Public: true
--    - File size limit: 10MB
--    - Allowed MIME types: image/jpeg, image/png, image/webp
--
-- 3. 'session-media' - For session banner images and promo videos
--    - Public: true
--    - File size limit: 500MB
