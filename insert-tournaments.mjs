...
async function insertTournaments() {
  try {
    ...
    orgs.forEach((org, i) => console.log(`  ${i + 1}. ${org.name} (ID: ${org.id})`));
    ...
    console.log('\nInserting 10 example tournaments...');
    ...
    console.log(`\n✅ Successfully added ${data.length} tournaments!`);
    console.log('\nTournament Summary:');
    data.forEach((t, i) => {
      console.log(`  ${i + 1}. ${t.title} (${t.sport}) - ${t.city}, ${t.state}`);
    });
