...
export default function PageEditor({
  ...
  const showRulers = showRulersProp ?? false;
  const showGrid = showGridProp ?? true;
  const snapToGrid = snapToGridProp ?? true;
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 bg-slate-950 overflow-y-auto relative">
        {/* Horizontal Ruler */}
        {showRulers && (
          <div className="sticky top-0 left-0 right-0 h-6 bg-slate-800 border-b border-slate-700 flex items-end z-10">
              ...
              {Array.from({ length: 50 }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {i % 2 === 0 && (
                      <span className="text-xs text-slate-400 mt-0.5">{position}</span>
                    )}
                  </div>
                );
              })}
        ...
        {/* Vertical Ruler */}
        {showRulers && (
          <div className="fixed left-0 top-0 bottom-0 w-6 bg-slate-800 border-r border-slate-700 flex flex-col justify-start z-10" style={{ marginTop: '24px' }}>
            {Array.from({ length: 100 }).map((_, i) => {
              ...
              return (
                ...
                >
                  <div className="w-2 h-px bg-slate-500 ml-auto"></div>
                  {i % 2 === 0 && (
                    <span className="text-xs text-slate-400 ml-0.5 transform -rotate-90 origin-left whitespace-nowrap">{position}</span>
                  )}
                </div>
              );
            })}
        ...
        {/* Canvas Content */}
        <div
          className={`mx-auto transition-all duration-300 ${showRulers ? 'ml-6 mt-6' : ''} ${
            viewMode !== 'desktop' ? 'shadow-2xl border-4 border-slate-700' : ''
          }`}
          style={{
          ...
          {sections.length === 0 ? (
            ...
            sections.map((section) => {
              ...
              return (
                <div
                  key={section.id}
                  className={`relative border-b-2 border-slate-800 transition-all ${
                    isSelectedSection ? 'ring-2 ring-blue-500' : ''
                  }`}
                  style={{
